...
namespace DemoShop.Web.PaymentHelpers
{
    public static class DirectPaymentsHelper
    {
        ...
        public static dynamic ProcessPaymentResult(string data, string siteKey, string codexApiKey)
        {
            try
            {
                ...
                // (the codex api provides us with the bank's key)
                ...
                // verify the PaymentProof to make sure the provided bank key is trusted by the codex api
                bool isPaymentProofSignatureValid = VerifySignature(paymentProofJson,
                    paymentProofSignature, codexApiKey);
